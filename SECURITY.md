...
- Require an allowlisted `Host`, a bounded delivery identifier, and a per-source webhook rate limit.
...
- Add deployment-edge replay-window checks and a durable delivery ledger with delivery timestamps.
- Replace the in-process rate limiter with edge-backed limits for multi-instance deployments.
